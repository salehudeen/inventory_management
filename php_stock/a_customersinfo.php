...
// End of mofidication Flexibility of Export Records Options, by Deen Saleh, May 14, 2012
	...
	function KeyUrl($url, $parm = "") {
		...
		if (!is_null($this->Customer_ID->CurrentValue)) {
			...
			return "javascript:alertify.alert(ewLanguage.Phrase('InvalidRecord'), function (ok) { }).set('title', ewLanguage.Phrase('AlertifyAlert'));"; // Modification Alertify by Deen Saleh, October 14, 2013
		}
		return $sUrl;
	}
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				// Begin of modification Add Record Number Column on Exported List, modified by Deen Saleh, June 3, 2012
				...
				// End of modification Add Record Number Column on Exported List, modified by Deen Saleh, June 3, 2012
					if ($this->Customer_Number->Exportable) $Doc->ExportCaption($this->Customer_Number);
					if ($this->Customer_Name->Exportable) $Doc->ExportCaption($this->Customer_Name);
					if ($this->Contact_Person->Exportable) $Doc->ExportCaption($this->Contact_Person);
					if ($this->Phone_Number->Exportable) $Doc->ExportCaption($this->Phone_Number);
